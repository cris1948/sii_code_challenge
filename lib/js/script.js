...
var MOVIEDB_API_KEY = 'fff2f288005425652515f2361d8b1964';
var MOVIEDB_BASE_URL = 'https://api.themoviedb.org/3';
var MOVIEDB_IMAGES_URL = 'https://image.tmdb.org/t/p/w780/';
...
var MovieDB = {
  ...
  page: 0,
  query: null,
  makeRequest: function makeRequest(method, data) {
  ...
  createQueryString: function createQueryString(data) {
    var string = '?api_key=' + MOVIEDB_API_KEY;
    if (data) {
  ...
  listenerAction: function listenerAction(textQuery) {
  ...
  searchMovie: function searchMovie(textQuery) {
    var _this = this;

    var page = void 0;

    if (this.results || this.query !== null && textQuery !== this.query) {
      var totalPages = this.results.total_pages;
    ...
    var query = { query: textQuery, page: page };

    this.makeRequest('searchMovie', query).then(function (response) {
      _this.results = response;
      _this.renderResultsList(response.results);
      document.querySelector('.spinner').style.display = 'none';
      document.querySelector('.pages').innerText = ' ' + page + ' / ' + response.total_pages;
    });
  },
  authenticateUser: function authenticateUser(textQuery) {
    var _this2 = this;

    this.makeRequest('getToken').then(function () {
      return _this2.makeRequest('createGuestSession');
    }).then(function (response) {
      _this2.guestSessionId = response.guest_session_id;
    }).then(function () {
      _this2.searchMovie(textQuery);
    }).catch(function (error) {
      console.log(error);
    });
  },
  renderResultsList: function renderResultsList(results) {
    var container = document.getElementById('results');
    container.innerHTML = '';

    results.forEach(function (result) {
      var imageUrl = result.backdrop_path ? '' + MOVIEDB_IMAGES_URL + result.backdrop_path : 'https://assets.tmdb.org/images/v4/logos/91x81.png';
...
window.onload = function () {
  ...
  buttons.forEach(function (button) {
    button.addEventListener('click', function (evt) {
      var btnType = evt.target.getAttribute('data-page');
      if (btnType < 0) {
        MovieDB.page -= 2;
      }
  ...
  searchInput.addEventListener('keyup', function (evt) {
    ...
    var helper = document.querySelector('.search__helper');
